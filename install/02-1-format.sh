...
do
    mkdir -p "/mnt/$vol"
    mount -o "ssd,noatime,space_cache=v2,autodefrag,compress=zstd:15,discard=async,subvol=@${vol//\//_}" "$BTRFS" "/mnt/$vol"
done
...
mount -o "ssd,noatime,space_cache=v2,autodefrag,compress=zstd:15,discard=async,subvol=@/boot" "$BTRFS" "/mnt/boot"
